...
import 'package:mini_vtop/browser/models/web_archive_model.dart';
import 'package:mini_vtop/browser/models/webview_model.dart';
...
class BrowserModel extends ChangeNotifier {
  ...
  bool containsFavorite(FavoriteModel favorite) {
    return _favorites.contains(favorite) ||
        _favorites.map((FavoriteModel? e) => e).firstWhere(
  ...
  void removeFavorite(FavoriteModel favorite) {
    if (!_favorites.remove(favorite)) {
      var favToRemove = _favorites.map((FavoriteModel? e) => e).firstWhere(
          (element) => element!.url == favorite.url,
          orElse: () => null);
