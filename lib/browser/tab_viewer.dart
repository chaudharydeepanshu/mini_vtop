...
class ScrollableTab extends StatefulWidget {
  ...
  const ScrollableTab({Key? key, required this.child, this.top = 0.0, this.onTap})
      : super(key: key);
...
class _TabViewerState extends State<TabViewer>
    ...
    for (var i = 0; i < widget.children.length; i++) {
      if (widget.currentIndex == i) {
        if (widget.currentIndex == 0) {
          positions[widget.currentIndex] = tabViewerTopOffset1;
        } else if (widget.currentIndex == 1) {
          positions[widget.currentIndex] = tabViewerTopOffset2;
        } else if (widget.currentIndex >= 2) {
          positions[widget.currentIndex] = tabViewerTopOffset3;
        }
      } else {
        if (i < widget.currentIndex) {
          if (i == 0) {
            positions[i] = tabViewerTopOffset1;
          } else if (i == 1) {
            positions[i] = tabViewerTopOffset2;
          } else if (i >= 2) {
            positions[i] = tabViewerTopOffset3;
          }
        } else {
          if (i == positions.length - 1) {
            positions[i] =
                MediaQuery.of(context).size.height - tabViewerBottomOffset1;
  ...
  void didUpdateWidget(TabViewer oldWidget) {
    ...
    if (diffLength > 0) {
      ...
      if (positions.length == 1) {
        positions[0] = tabViewerTopOffset1;
      }
    }
  }
            ...
            children: widget.children.map((tab) {
              ...
              if (index == focusedIndex &&
                ...
                opacity = 0.15;
                      ...
                      positions[index] <= tabViewerTopOffset3) ||
              ...
              double scale = 1.0;
              if (positions[index] < tabViewerTopScaleTopOffset) {
                scale =
                    (positions[index] / tabViewerTopScaleTopOffset) + 0.85;
                if (scale > 1) {
                  scale = 1.0;
                }
              } else if (positions[index] >
                  MediaQuery.of(context).size.height -
                      tabViewerTopScaleBottomOffset) {
                var diff = MediaQuery.of(context).size.height -
                    tabViewerBottomOffset1 -
                    positions[index];
                scale = (diff / tabViewerTopScaleBottomOffset) + 0.7;
                if (scale > 1) {
                  scale = 1.0;
                }
              } else {
  ...
  void updatePositions(double dy) {
    ...
    if (focusedIndex == 0 &&
        positions[focusedIndex] <= tabViewerTopOffset1) {
      positions[focusedIndex] = tabViewerTopOffset1;
      focusedIndex = min(positions.length - 1, focusedIndex);
    } else if (focusedIndex == 1 &&
        positions[focusedIndex] < tabViewerTopOffset2) {
      positions[focusedIndex] = tabViewerTopOffset2;
      focusedIndex = min(positions.length - 1, focusedIndex + 1);
    } else if (focusedIndex >= 2 &&
        positions[focusedIndex] < tabViewerTopOffset3) {
      positions[focusedIndex] = tabViewerTopOffset3;
      focusedIndex = min(positions.length - 1, focusedIndex + 1);
